...
try:
    ...
    _ACCURACY_ENHANCEMENTS_AVAILABLE = True
except ImportError:
    _ACCURACY_ENHANCEMENTS_AVAILABLE = False
    _logging.getLogger(__name__).info("Accuracy enhancements not loaded (improve_accuracy.py not found)")
...
def compute_delay_prediction(
    ...
    if ml_bundle is not None and not (holding_detected or go_around_detected or diversion_suspected):
        try:
            ...
            # ── Apply accuracy enhancements (holidays, fog, live traffic) ──────
            if _ACCURACY_ENHANCEMENTS_AVAILABLE and _ist_dt is not None:
                try:
                    today_str = _ist_dt.strftime("%Y-%m-%d")
                    is_holiday = int(is_public_holiday(today_str))
                    is_adj = int(is_holiday_adjacent(today_str))
                    is_fest = int(is_festival_season(today_str))
                    is_fog = int(
                        _ist_dt.month in [12, 1, 2] and
                        (dep_icao_ml in FOG_SEASON_AIRPORTS or arr_icao_ml in FOG_SEASON_AIRPORTS)
                    )
                    # Live traffic score (use route congestion as proxy)
                    live_traffic = min(metric.get("inbound_density", 5), 10) if metric else 5.0
                    ...
                    multiplier = calculate_delay_multiplier(
                        ...
                        is_fog_alert=is_fog,
                        live_traffic_score=live_traffic
                    )

                    # Apply multiplier to ML probability
                    ml_prob = min(ml_prob * multiplier, 0.95)

                    # Add reason tags for significant factors
                    if is_holiday:
                        reasons.append("public holiday travel surge")
                    elif is_adj:
                        reasons.append("holiday-adjacent high demand")
                    if is_fest:
                        reasons.append("festival season rush")
                    if is_fog:
                        reasons.append("fog season alert")
                    if multiplier > 1.3:
                        reasons.append(f"enhanced delay risk ({multiplier:.1f}x)")
                except Exception:  # noqa: BLE001
                    pass  # Enhancements failed, use base ML prob
