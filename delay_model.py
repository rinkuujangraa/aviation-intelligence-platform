...
FEATURE_NAMES = [
    ...
    "is_public_holiday",       # 1 if Indian public holiday
    "is_holiday_adjacent",     # 1 if within 1 day of holiday
    "is_festival_season",      # 1 if Diwali/Christmas/NY season
    "is_fog_alert",            # 1 if fog-prone airport in winter
    "dep_dow_x_is_weekend",    # interaction: day-of-week × weekend
    "is_peak_x_route_count",   # interaction: peak hour × route frequency
    "monsoon_x_arr_airport",   # interaction: monsoon × destination (encoded)
]
...
def build_features(
    ...
    if _enhancements_available and "dep_date" in df.columns:
        ...
        df["is_holiday_adjacent"] = 0
        df["is_festival_season"] = 0

    # Fog alert feature
    if _enhancements_available:
        df["is_fog_alert"] = (
            df["dep_month"].isin([12, 1, 2]) &
            df["dep_icao"].isin(FOG_SEASON_AIRPORTS)
        ).astype(int)
    else:
        df["is_fog_alert"] = 0

    # Interaction features (capture non-linear relationships)
    df["dep_dow_x_is_weekend"] = df["dep_dow"] * df["is_weekend"]
...
def predict_delay_prob(
    ...
    # ── Enhancement features (holiday, fog, etc.) ────────────────────────────
    is_weekend_val = 1 if dep_dow >= 5 else 0
    is_peak_hour_val = 1 if (7 <= dep_hour_ist <= 10 or 17 <= dep_hour_ist <= 21) else 0

    # Try to import enhancement functions
    try:
        from improve_accuracy import (
            is_public_holiday,
            is_holiday_adjacent,
        ...
        is_pub_holiday = int(is_public_holiday(today_str))
        is_hol_adj = int(is_holiday_adjacent(today_str))
        is_fest = int(is_festival_season(today_str))
        is_fog = int(dep_month in [12, 1, 2] and dep_icao in FOG_SEASON_AIRPORTS)
    except ImportError:
        is_pub_holiday = 0
        is_hol_adj = 0
        is_fest = 0
    ...
    row = pd.DataFrame([{
        ...
        "is_holiday_adjacent": is_hol_adj,
        "is_festival_season": is_fest,
        "is_fog_alert": is_fog,
        "dep_date": None,  # not available during inference
    }])
